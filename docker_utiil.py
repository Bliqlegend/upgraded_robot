...
def options():
    ...
    print("Options Menu  : \n")
    l = ["Deploy an Instance","Copy Code files to Backupdir","Exec into terminal","Run a certain Command","Delete any instance","Get container IP","SSH into the container","Commit to DockerHub","Check How many instances are running","Deploy a temporary instance"]
    n =1
    for i in l:
        print(f"{n}. {i}")
        n+=1
...
def deploy_an_instance(name):
    ...
    os.system("docker run -dit --privileged --security-opt seccomp=unconfined -h {$USER} -p 8080:80 -p 2222:22 -e ALLOWED_NETWORKS=0.0.0.0/0 cultholmes/progressive_minds")
    print("Done")
    num = options()
    return num
...
def get_continer_ip(name):
    ...
    l = os.popen("docker inspect --format='{{.Id}}' "+ name)
    brr = l.read()
    os.system("docker inspect -f '{{range.NetworkSettings.Networks}}{{.IPAddress}}{{end}}' "+ name)
    num = options()
    return num
...
def deploy_a_temp_instance(name):
    ...
    print("Deploying an instance with Arbitraty name and 8080,2222")
    os.system(" docker pull cultholmes/progressive_minds")
    os.system("docker run -it --rm --privileged --security-opt seccomp=unconfined -h {$USER} -p 8080:80 -p 2222:22 -e ALLOWED_NETWORKS=0.0.0.0/0 cultholmes/progressive_minds")
    print("Done")
    num = options()
    return num
